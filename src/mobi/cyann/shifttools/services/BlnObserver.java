 ...
 */
package mobi.cyann.shifttools.services;
...
public class BlnObserver extends FileObserver {
	private final static String LOG_TAG = "ShiftTools.BlnObserver";
	...
	public BlnObserver(Context context) {
		...
		context.registerReceiver(alarmReceiver, new IntentFilter("mobi.cyann.shifttools.SHUTDOWN_BLN"));
	}
		...
		if(event == FileObserver.CLOSE_WRITE) {
			...
			if(blnTimeout > 0) {
				...
				if(n > 0 && sc.getLastResult(0).equals("1")) { // BLN on
					...
					PendingIntent pendingIntent = PendingIntent.getBroadcast(context, 1, new Intent("mobi.cyann.shifttools.SHUTDOWN_BLN"), PendingIntent.FLAG_CANCEL_CURRENT);
