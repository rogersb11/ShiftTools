...
public class PreloadValues {
	private final static String LOG_TAG = "ShiftTools.PreloadValues";
	...
	public void reload() {
		prop = new Properties();
		try {
			FileInputStream fis = new FileInputStream("/data/data/mobi.cyann.shifttools/preload.prop");
			prop.load(fis);
			fis.close();
		}catch(Exception ex) {
			Log.e(LOG_TAG, "fail to read preload file", ex);
			SysCommand sc = SysCommand.getInstance();
			int r = sc.suRun("chmod", "0755", "/data/data/mobi.cyann.shifttools/preload.prop");
			if(r < 0) {
